...
public class DAO implements Disposable {
	...
	public List querySQL(String requestedQuery, Class entityClass) {
		...
		try {
			...
			Query query;
			if (entityClass == null) {
				query = querySession.createSQLQuery(requestedQuery);
			} else {
				query = querySession.createSQLQuery(requestedQuery).addEntity(entityClass);
			}
	...
	private void handleExceptionDuringTransaction(Exception ex, Transaction transactionWithException) {
		...
		LOG.error(" ---> An exception occurred! Last transaction was rolled back!\n\n" + ExceptionsFormatter.toString(ex));
	}
}
