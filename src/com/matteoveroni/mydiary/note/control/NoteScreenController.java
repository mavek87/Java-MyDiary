...
public class NoteScreenController implements Initializable, Manageable, Listener {
    ...
    @FXML
    void menuExportClicked(ActionEvent event) {
        FileChooser fileChooser = new FileChooser();
        fileChooser.setTitle(resourceBundle.getString("exportThisNoteToAFile"));

        FileChooser.ExtensionFilter extFilter = new FileChooser.ExtensionFilter("HTML files (*.htm)", "*.htm", "(*.html)", "*.html");
        fileChooser.getExtensionFilters().add(extFilter);

        File exportNoteFile = fileChooser.showSaveDialog(null);

        try (FileWriter fileWriter = new FileWriter(exportNoteFile)) {
            String noteMessage = ExtractHTMLNoteMessage();
            String noteTitle = ExtractTXTNoteTitle();

            String HTMLNoteHeader = noteMessage.substring(0, 48);
            String HTMLNoteWithoutHeader = noteMessage.substring(48, noteMessage.length());
            String HTMLNoteTitle = "<h1 style=\"text-align: center;\">" + noteTitle + "</h1>";

            String fullHTMLNoteTitleAndMessage = HTMLNoteHeader + HTMLNoteTitle.toUpperCase() + "<br />" + HTMLNoteWithoutHeader;

            LOG.debug(fullHTMLNoteTitleAndMessage);

            fileWriter.write(fullHTMLNoteTitleAndMessage);

        } catch (IOException ex) {
            JOptionPane.showMessageDialog(
                null,
                resourceBundle.getString("noteNotExportedMessage"),
                resourceBundle.getString("noteNotExported"),
                JOptionPane.ERROR
            );
            return;
        }
        ...
        JOptionPane.showMessageDialog(
            null,
            resourceBundle.getString("noteExportedMessage"),
            resourceBundle.getString("noteExported"),
            JOptionPane.INFORMATION_MESSAGE
        );
...
//		}
    ...
    private boolean isCurrentNoteSaved() {
        boolean isNoteSaved;
        try {
            isNoteSaved = currentNote.getMessage().equals(ExtractHTMLNoteMessage()) && currentNote.getTitle().equals(ExtractTXTNoteTitle());
        } catch (Exception ex) {
            isNoteSaved = false;
        }
        return isNoteSaved;
    ...
    private String ExtractTXTNoteTitle() {
